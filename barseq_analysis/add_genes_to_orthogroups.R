...
locustag_matrix <- read.delim("dickeya_db/locustag_matrix.txt", header = T)

# Import RefSeq "feature tables" to match protein ID ("product_accession") with "locus_tag".
features_Dda3937 <- read.delim("dickeya_db/Dda3937_feature_table.txt", header = T)
features_DdiaME23 <- read.delim("dickeya_db/DdiaME23_feature_table.txt", header = T)
features_Ddia6719 <- read.delim("dickeya_db/Ddia67_19_feature_table.txt", header = T)
...
# Save output.
write.csv(locustag_matrix, "dickeya_db/locustag_matrix_with_genes.txt",
          row.names = F)
